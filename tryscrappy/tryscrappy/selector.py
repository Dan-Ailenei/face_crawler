...
from tryscrappy.utils import convert_date, compare_url_existing_parameters
...
class PageSelector:
    def __init__(self, SUBJECT):
        ...
        self.profile_regex = re.compile("fotografia.*?profil.*?este posibil ca imaginea să conţină:.*?\d (\w+) ?, ",
                                        flags=re.I)
    ...
    def _find_sibling_url(self, trace, person, i):
        if i == 0:
            return
        current_trace = trace.parent
        result = current_trace.find(href=lambda href: self._re_url_filter(href, person, self.url_regex))
        if result:
            return result
        return self._find_sibling_url(current_trace, person, i - 1)
    ...
    def _select_siblings(self, soup, person):
        ...
        for sibling in self.siblings:
            ...
            for trace in traces:
                result = self._find_sibling_url(trace, person, 3)
                if result:
